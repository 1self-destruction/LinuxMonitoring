...
function info_about_color {
    reg="^[1-6]$"
    echo -e "\n"
    echo "Column 1 background = $(if [[ $1 -eq 1 ]] ; then echo "default (black)" ; else echo -n ${column1_background}; print_color_text ${column1_background}; fi)"
    echo "Column 1 font_color = $(if [[ $1 -eq 1 ]] ; then echo "default (white)" ; else echo -n ${column1_font_color}; print_color_text ${column1_font_color}; fi)"
    echo "Column 2 background = $(if [[ $1 -eq 1 ]] ; then echo "default (black)" ; else echo -n ${column2_background}; print_color_text ${column2_background}; fi)"
    echo "Column 2 font_color = $(if [[ $1 -eq 1 ]] ; then echo "default (white)" ; else echo -n ${column2_font_color}; print_color_text ${column2_font_color}; fi)"     
}
